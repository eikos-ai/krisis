...
Before generating a response, Metis runs a planning phase (using Haiku) that reformulates the user's query into optimized search terms. These reformulated terms drive memory retrieval instead of the raw message, so vague references like "that CrewAI thing" resolve to the right stored context. The planning trace and retrieved memories — along with auto-generated project narrative (synthesized by the main model from structured project facts) from `<project_knowledge>` — feed into the main generation call.
